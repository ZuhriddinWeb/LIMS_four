...
class LabProductsController extends Controller
{
    ...
    {
        return response()->json(LabProduct::all());
    }

    private function getRow($id)
    {
        return response()->json(LabProduct::find($id));
    }
    ...
    private function create(Request $request)
    {
        $data = $request->validate([
            ...
            'ProductType' => 'nullable|string|max:50',
            'Comment' => 'nullable|string|max:1000',
        ]);

        $row = LabProduct::create($data);
    ...
    {
        $data = $request->validate([
            'id' => 'required|integer|exists:lab_products,id',
            'Code' => 'nullable|string|max:255',
            'Name' => 'required|string|max:255',
            'NameRus' => 'nullable|string|max:255',
            'ShortName' => 'nullable|string|max:255',
        ...
        $row = LabProduct::find($request->id);
        $row->update($data);
    ...
    {
        try {
            LabProduct::findOrFail($id)->delete();
