...
class LabSampleTypesController extends Controller
{
    ...
    {
        return response()->json(LabSampleType::all());
    }

    private function getRow($id)
    {
        return response()->json(LabSampleType::find($id));
    }
    ...
    {
        $data = $request->validate([
            'id' => 'required|integer|exists:lab_sample_types,id',
            'Code' => 'nullable|string|max:255',
            'Name' => 'required|string|max:255',
            'NameRus' => 'nullable|string|max:255',
            'ShortName' => 'nullable|string|max:255',
        ...
        $row = LabSampleType::find($request->id);
        $row->update($data);
