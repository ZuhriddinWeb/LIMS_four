<template>
    <div class="p-3 pt-28">
        <div class="flex items-center gap-3 mb-3">
            ...
            <VaDateInput v-model="date" @update:modelValue="onDateChanged" :teleport="true" teleport-target="body"
...
async function postStaticForCell(addr, { toast = false } = {}) {
  ...
  // ① Avval xom qiymatni olamiz
  let raw = jexcel.getValueFromCoords(x, y, false)

  // ② Agar xom qiymat formula bo‘lsa (= ...), yoki bo‘sh bo‘lsa — hisoblanganini oling
  if (
    raw == null ||
    raw === '' ||
    (typeof raw === 'string' && raw.trim().startsWith('='))
  ) {
    // jSpreadsheet’ga hisoblashga fursat berish (ayniqsa yangi setValue dan keyin)
    await new Promise(r => requestAnimationFrame(r))
    raw = jexcel.getValueFromCoords(x, y, true) // <-- computed/result
  }

  const value = toNumber(raw)
  if (!Number.isFinite(value)) return  // #ERROR yoki matn bo‘lsa, saqlamaymiz
...
function toNumber(val) {
  ...
  let s = String(val).trim().replace(/^\s*=/, '') // "=..." ni olib tashlaymiz
  s = s.replace(/\u00A0/g, '')
  const lastDot = s.lastIndexOf('.')
  const lastComma = s.lastIndexOf(',')
  const decimalSep = Math.max(lastDot, lastComma) === -1 ? '' : (lastDot > lastComma ? '.' : ',')
  s = s.replace(/\s+/g, '')
...
const onDateChanged = async () => {
  await fetchStaticParams() // tanlangan sana bilan backenddan qayta o‘qish
  await build()
}
