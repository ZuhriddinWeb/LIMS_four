<template>
  <main class="h-full w-full text-center content-center">
    <VaButton round icon="folder_delete" preset="primary" class="mt-1" @click="selectedDataDelete = true" />
    <VaModal 
...
const onSubmit = async () => {
  // console.log(props.params.data['ParametersID']);
  
  try {
    const { data } = await axios.delete(`/calculator/${props.params.data['ParametersID']}`);
    if (data.status === 200) {
