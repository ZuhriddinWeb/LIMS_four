...
class LabLaboratoriesController extends Controller
{
    ...
    {
        return response()->json(LabLaboratory::all());
    }

    private function getRow($id)
    {
        return response()->json(LabLaboratory::find($id));
    }
    ...
    {
        $data = $request->validate([
            'id' => 'required|integer|exists:lab_laboratories,id',
            'Code' => 'nullable|string|max:255',
            'Name' => 'required|string|max:255',
            'NameRus' => 'nullable|string|max:255',
            'ShortName' => 'nullable|string|max:255',
        ...
        $row = LabLaboratory::find($request->id);
        $row->update($data);
    ...
    {
        try {
            LabLaboratory::findOrFail($id)->delete();
